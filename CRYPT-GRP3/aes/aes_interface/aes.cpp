...
//    uint8_t temp_1[4] = {gmult(a[0],b[0]), gmult(a[1],b[0]),  gmult(a[2],b[0]), gmult(a[3],b[0])};
//    uint8_t temp_2[4] = {gmult(a[3],b[1]), gmult(a[0],b[1]),  gmult(a[1],b[1]), gmult(a[2],b[1])};
//    uint8_t temp_3[4] = {gmult(a[2],b[2]), gmult(a[3],b[2]),  gmult(a[0],b[2]), gmult(a[1],b[2])};
//    uint8_t temp_4[4] = {gmult(a[1],b[3]), gmult(a[2],b[3]),  gmult(a[3],b[3]), gmult(a[0],b[3])};
//
//    *(uint32_t *)d = (*(uint32_t *)temp_1)^(*(uint32_t *)temp_1)^(*(uint32_t *)temp_3)^(*(uint32_t *)temp_4);
}
...
void shift_rows(uint8_t *state) {
    ...
    for (i = 1; i < 4; i++) {
        ...
        for (k = i; k < Nb; k++) {
            state[Nb*i+k-i] = state[Nb*i+k];
        }
        for(s = 0; s < i ; s++) {
            state[Nb * i + Nb - i + s] = tmp[s];
        }
    }
}
#else
...
void aes_inv_cipher(uint8_t *in, uint8_t *out, uint8_t *w) {
    ...
    for (i = 0; i < 4; i++) {
        for (j = 0; j < Nb; j += 4) {
            out[i + 4 * j] = state[Nb * i + j];
            out[i + 4 * (j + 1)] = state[Nb* i + j + 1];
            out[i + 4 * (j + 2)] = state[Nb* i + j + 2];
            out[i + 4 * (j + 3)] = state[Nb* i + j + 3];
        }
    }
}