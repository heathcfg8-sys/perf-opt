...
// 连续旋转因子存储区：减少每层单独 malloc 带来的动态分配开销
static double *array_r_storage = NULL;
static double *array_i_storage = NULL;
...
void preCompute(int size) {
    ...
    // 释放上一次预计算的旋转因子表，避免连续测试不同 size 时产生内存泄漏
    if (array_r != NULL) {
        free(array_r);
        array_r = NULL;
    }
    ...
    if (array_r_storage != NULL) {
        free(array_r_storage);
        array_r_storage = NULL;
    }
    if (array_i_storage != NULL) {
        free(array_i_storage);
        array_i_storage = NULL;
    }

    array_r = (double**)malloc(sizeof(double*) * max_level);
    array_i = (double**)malloc(sizeof(double*) * max_level);

    // 所有层的旋转因子数量总和为 1 + 2 + 4 + ... + size/2 = size - 1
    array_r_storage = (double*)malloc(sizeof(double) * (size - 1));
    array_i_storage = (double*)malloc(sizeof(double) * (size - 1));

    if (array_r == NULL || array_i == NULL || array_r_storage == NULL || array_i_storage == NULL) {
        printf("Error: Allocate twiddle factor memory failed\\n");
        return;
    }
    ...
    for (int le = 2; le <= size; le <<= 1, level++) {
        ...
        // 当前层指向连续存储区中的一段，避免每层单独 malloc
        array_r[level] = array_r_storage + offset;
        array_i[level] = array_i_storage + offset;
        offset += le2;
